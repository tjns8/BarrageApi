...
public class Barrage extends AbsoluteLayout implements Callback {
	...
	private static String Ip = "http://192.168.1.105/phpserver/src/BarrageApi.php";
			...
			public void run() {
				...
				try {
					...
					for(String m:msgs){
						...
						int ils=m.indexOf(" ");
						barrageView.start=Long.parseLong(m.substring(0, ils));
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					httpPost.addHeader("text", textutf8);
					httpPost.addHeader("start", strstart);
					HttpClient client = new DefaultHttpClient();
					Log.i("send:", textutf8);
